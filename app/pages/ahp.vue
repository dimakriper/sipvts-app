<template>
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-2">
      Метод Аналитической Иерархии (Analytic Hierarchy Process)
    </h1>
    <p class="text-gray-600 dark:text-gray-300 mb-6">
      Многокритериальный анализ принятия решений с проверкой согласованности
    </p>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
      ...
      <ol class="text-sm text-blue-700 dark:text-blue-400 space-y-1 list-decimal list-inside">
        ...
        <li>Проверьте <strong>Отношение согласованности (ОС)</strong>: значение &le; 10&#8239;% считается допустимым. Если ОС &gt; 10&#8239;%, пересмотрите оценки.</li>
      </ol>
    </div>
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2 mb-3">
                <div
                  v-for="(criterion, index) in store.criteria"
                  :key="`crit-${index}`"
        ...
        <div class="lg:col-span-3">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Оцените важность каждого критерия (1–9, где 1 = равные)
            </p>

            <div
              v-if="store.criteria.length > 0"
            ...
            >
              <table class="w-full text-sm">
                ...
                <tbody>
                  <tr
                    v-for="(criterion, i) in store.criteria"
                    :key="`row-${i}`"
                  ...
                  >
                    ...
                    <td
                      v-for="(criterion2, j) in store.criteria"
                      :key="`cell-${i}-${j}`"
                    ...
                    >
                      ...
                      <span
                        v-else
                        class="text-gray-500"
                      >{{ formatReciprocal(store.criteriaMatrix[`${j}-${i}`] || 1) }}</span>
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div
            v-for="(criterion, cIdx) in store.criteria"
            :key="`alt-matrix-${cIdx}`"
            class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6"
          >
            <h3 class="text-lg font-semibold mb-4">
              {{ criterion }} — Сравнение альтернатив
            </h3>
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Оцените альтернативы по данному критерию (1–9)
            </p>

            <div
              v-if="store.alternatives.length > 0"
              class="overflow-x-auto"
            >
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-gray-300 dark:border-gray-700">
                    ...
                    <th
                      v-for="(alt, idx) in store.alternatives"
                      :key="`header-alt-${idx}`"
                      class="text-center p-2 font-semibold"
                    >
                      {{ alt.substring(0, 8) }}
                ...
                <tbody>
                  <tr
                    v-for="(alternative, i) in store.alternatives"
                    :key="`alt-row-${i}`"
                    class="border-b border-gray-300 dark:border-gray-700"
                  >
                    <td class="p-2 font-medium">
                    ...
                    <td
                      v-for="(alternative2, j) in store.alternatives"
                      :key="`alt-cell-${i}-${j}`"
                      class="text-center p-2"
                    >
                      <select
                      ...
                      <span
                        v-else
                        class="text-gray-500"
                      >{{ formatReciprocal(getAltValue(cIdx, `${j}-${i}`) || 1) }}</span>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div class="grid grid-cols-2 gap-4 mb-8">
              ...
              <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded">
                ...
                <p
                  ...
                  :class="(results?.cr ?? 0) <= 0.1 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'"
                >
                  {{ ((results?.cr ?? 0) * 100).toFixed(2) }}%
                </p>
                <p
                  class="text-xs text-gray-500 mt-1"
                  :class="(results?.cr ?? 0) <= 0.1 ? 'text-green-600' : 'text-red-600'"
                >
